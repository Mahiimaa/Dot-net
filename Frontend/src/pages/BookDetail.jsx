    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const bookResponse = await api.get(
          `http://localhost:5127/api/Books/${bookId}`
        );
        ...
        if (stableIsAuthenticated) {
          ...
          try {
            const purchaseCheck = await api.get(
              `http://localhost:5127/api/Reviews/has-purchased/${bookId}`
            );
          ...
          try {
            const wishlistResponse = await api.get(
              `http://localhost:5127/api/Wishlist/user/${user?.id}`,
              {
                headers: {
            ...
            setWishlisted(
              wishlistResponse.data.some((b) => b.bookId === bookId)
            );
          } catch (wishlistErr) {
            console.warn(
              "Wishlist fetch failed:",
              wishlistErr.response?.status,
  ...
  const toggleWishlistStatus = async () => {
    ...
    try {
      if (wishlisted) {
        ...
        if (wishlistItem) {
          await api.delete(
            `http://localhost:5127/api/Wishlist/remove/${wishlistItem.id}`,
            {
              headers: {
                Authorization: `Bearer ${localStorage.getItem("token")}`,
              },
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-1">
            ...
            <div className="mt-6 space-y-4">
              ...
              >
                <Heart
                  className={`w-5 h-5 ${wishlisted ? "fill-white" : ""}`}
                />
              ...
              <div className="flex gap-4">
                <button
                  ...
                  onClick={addToCart}
                  disabled={
                    isAddingToCart || book?.availability !== "Available"
                  }
